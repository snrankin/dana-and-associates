...
if (!function_exists('gt3_footer_area')) {
    function gt3_footer_area()
    {
        ...
        if ($footer_switch || $copyright_switch || $pre_footer_switch) {
            $footer = '<footer id="footer" itemscope itemprop="WPFooter"  role="contentinfo">';

            if ($pre_footer_switch && !empty($pre_footer_editor)) {
                $pre_footer = '';
                ob_start(); ?>
<div id="pre-footer" class="section-wrapper bg-primary  d-print-none">
...
<?php $pre_footer = ob_get_clean();
            ...
            if ($footer_switch) {
                $main_footer = '';
                ob_start(); ?>
<div id="main-footer" class="section-wrapper bg-tertiary">
    <div class="section-inner">
        <div class="container">
            <div class="row grid-half">
                <?php for ($i = 0; $i < (int)$footer_column; $i++) {
                    ?>
                <div id="footer-col-<?php echo ($i + 1); ?>" class="footer-col col-ms-6 col-ml-4 col-lg-<?php echo $column_sizes[$i]; ?>">
                    <div class="content-wrapper">
                        <?php if ($mb_footer_switch == 'yes') {
                ...
                <div id="copyright" class="col-12">
                    <div class="content-wrapper">
                        <?php echo do_shortcode($copyright_editor); ?>
                        <?php } ?>
                    </div>
                </div>
            </div>
        </div>
        <?php $main_footer = ob_get_clean();
                $footer .= $main_footer;
            }
            $footer .= '</footer>';
            echo $footer;
