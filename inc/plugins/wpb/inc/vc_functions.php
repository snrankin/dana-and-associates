 ...
 * -----
 * Created Date: 4-2-19
 * Last Modified: 7-16-19 at 11:53 am
...
function maatVCClasses($css_classes, $base, $atts)
{
    $classes = '';
    if (!empty($css_classes)) {
        $css_class = preg_replace(
...
function maataddNewVCElement($elem, $atts, $params)
{
    // Stop all if VC is not enabled
    if (!defined('WPB_VC_VERSION')) {
        return;
    }
    $title    = str_replace("_maat", "", $elem);
    $title    = to_title_case($elem);
    $defaults = array(
        'name'                    => $title,
        'base'                    => 'vc_' . $elem,
        'is_container'            => false,
        ...
        'icon'                    => '',
        'class'                   => '',
        'html_template'           => PLUGINS_PATH . '/wpb/templates/vc_' . $elem . '.php',
        'params'                  => '',
    );
...
function maataddNewVCElementShortcode($input)
{
    $output = $input;
