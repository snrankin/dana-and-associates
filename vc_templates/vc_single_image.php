...
switch ($source) {
	...
	case 'featured_image':
		...
		$img['thumbnail'] = wp_get_attachment_image($img_id, $img_size);
	...
	case 'external_link':
		...
		$img = array(
			'thumbnail' => '<img class="img-fluid lazyload" ' . $hwstring . ' data-src="' . $custom_src . '" />',
		);
		break;
...
if (!$img) {
	$img['thumbnail'] = '<img class="img-placeholder single-img img-thumbnail" src="' . $default_src . '" />';
}
...
$wrapperClass = 'content-item image-wrapper ' . $style . ' ' . $border_color;
...
$class_to_filter = 'wpb_single_image content-item vc_align_' . $alignment . ' ' . $this->getCSSAnimation($css_animation);
